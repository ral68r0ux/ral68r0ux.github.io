...
const apiURL =
  "//api.openweathermap.org/data/2.5/forecast?lat=42.0372&lon=-111.3960&id=5604473&appid=802354a04208bdad04e551c4b5cb1f9f&units=imperial"; // city id = preston id
...
fetch(apiURL)
  ...
  .then((weatherInfo) => {
    ...
    document.getElementById("currentTemp").textContent =
      weatherInfo.list[0].main.temp + "\xB0" + "F";
    document.getElementById("currentWind").textContent =
      weatherInfo.list[0].wind.speed + " mph";
    document.getElementById("currentHumidity").textContent =
      weatherInfo.list[0].main.humidity + "%";

    const tempNumber = parseFloat(
      (weatherInfo.textContent = weatherInfo.list[0].main.temp)
    );
    const speedNumber = parseFloat(
      (weatherInfo.textContent = weatherInfo.list[0].wind.speed)
    );
    ...
    for (i = 0; i < facts.length; i++) {
      ...
      if (hours.includes("18:00:00")) {
        ...
        dayTemp.textContent = weatherInfo.textContent =
          weatherInfo.list[i].main.temp + "\xB0";
